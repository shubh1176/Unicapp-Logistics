    ...
    <div className='bg-[#fefae060]'>
      <div className='flex flex-col items-center gap-2  md:gap-10 border-solid bg-[#ffffffa3] px-4 md:px-8 pb-0 md:pb-5 pt-6 md:pt-16 mt-10 mb-10 mx-4 md:mx-12 lg:mx-24 rounded-3xl shadow-lg transition-all duration-300 ease-in-out'>
        ...
        <div className='grid grid-cols-2 gap-3 md:hidden mb-6 mt-6 mx-2 md:mx-8 md:px-12 lg:px-56 transition-all duration-300 ease-in-out'>
          <div className='flex flex-col items-center transition-all duration-300 ease-in-out'>
            ...
            <div className='relative bottom-2 flex items-center bg-black rounded-lg p-1 border border-black w-36 md:w-60 mt-1 hover:cursor-pointer transition-all duration-300 ease-in-out' onClick={() => router.replace('/Pickup-and-drop')}>
              <span className='flex flex-row justify-between gap-2 items-center'>
                ...
                <Button className="rounded-full w-6 h-6 text-sm bg-[#9E3CE1] border-white border border-spacing-1 p-1 hover:bg-[#9E3CE1] transition-all duration-300 ease-in-out">
                  <LucideArrowUpRight />
                </Button>
              </span>
            </div>
          ...
          <div className='flex flex-col items-center transition-all duration-300 ease-in-out'>
            <div className='bg-[#F8EBAB] h-28 w-36 md:h-60 md:w-60 rounded-2xl flex justify-center items-center transition-all duration-300 ease-in-out'>
              <Image className='w-20 h-20' src={'/images/delivery.png'} height={48} width={120} alt="Intercity Courier" />
            </div>
            <div className='relative bottom-2 flex items-center  bg-black rounded-lg p-1 border border-black w-36 md:w-60 mt-1 hover:cursor-pointer transition-all duration-300 ease-in-out' onClick={() => router.replace('/courier')}>
              <span className='flex flex-row justify-between items-center'>
                <span className='font-generalRegular text-white py-2 px-2 space-x-1 text-xs'>
                  Intercity Courier
                </span>
                <Button className="rounded-full w-6 h-6 border border-white bg-[#9E3CE1] border-spacing-1 p-1 hover:bg-[#9E3CE1] transition-all duration-300 ease-in-out" onClick={() => router.replace('/courier')}>
                  <LucideArrowUpRight />
                </Button>
              </span>
            </div>
