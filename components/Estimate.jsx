...
function EstimateComponent() {
  const [serviceType, setServiceType] = useRecoilState(orderTypeState);
  ...
  const calculateFare = async () => {
    if (serviceType === "Courier") {
      ...
      setAmount(parseFloat(totalAmount.toFixed(2)) || 0.00); // Ensure amount is a valid float
    } else {
  ...
  useEffect(() => {
    if (pickupCoords && dropCoords && mapContainerRef.current) {
      ...
      new mapboxgl.Marker({ color: "green" })
        ...
        .setPopup(new mapboxgl.Popup().setHTML(`<h3>Pickup: ${pickupAddress}</h3>`))
        .addTo(map);
      ...
      <div className="w-1/2 pr-4">
        ...
        <div className="text-lg mb-4 ml-5 w-4/5 translate-y-3">
          <div className="flex flex-row gap-3">
            <Image src="/images/fort.svg" height={25} width={25} alt="Delhi" />
            <p className="mb-1 text-sm translate-y-1">Delhi</p>
          </div>
          <p className="font-generalSemiBold text-2xl mt-3 mb-2">Intra-city deliveries</p>
          <p className="mb-4 font-generalRegular text-sm">
            For intra-city deliveries, we utilise our dedicated team of delivery captains to ensure prompt and reliable service.
          </p>
          <div className="flex flex-row gap-3 mt-8 mb-2">
            <Image src="/images/india.svg" height={25} width={25} alt="India" />
            <p className="mb-2 text-sm translate-y-1">All-over India</p>
          </div>
          <p className="font-generalSemiBold text-2xl mb-2">Inter-city deliveries</p>
          <p className="font-generalRegular text-sm">
            For intercity courier services, we partner with the nation's most trusted and reliable logistics companies to ensure timely and secure deliveries.
          </p>
        </div>
      </div>
      ...
      <div className="absolute right-0 transform -translate-x-5 translate-y-6 bg-white bg-opacity-25 rounded-xl min-h-28 max-w-full">
        <div className="bg-black bg-opacity-25 w-full h-full rounded-xl py-6 px-4 border-black border-2 border-opacity-5">
          <div className="flex flex-row gap-3 mb-4 w-auto mr-5">
            <div
              className={`relative flex flex-col px-4 py-2 w-80 rounded-2xl h-24 bg-opacity-50 pt-3 pl-5 ${serviceType === "Pickup & Drop" ? "bg-[#202020] bg-opacity-75 text-white" : "bg-[#202020] bg-opacity-25 text-white hover:bg-opacity-25"} border-white border-opacity-25 border-2`}
              onClick={() => {
            ...
            >
              <span className="text-xl font-generalMedium pt-1.5">Pickup & Drop</span>
              <div className="mt-1 opacity-50">
                <span className="mt-10 text-sm">For on-demand intracity deliveries</span>
              </div>
            ...
            <div
              className={`relative flex-1 px-5 py-2 rounded-2xl w-auto h-24 bg-opacity-50 ${serviceType === "Courier" ? "bg-[#202020] bg-opacity-75 text-white" : "bg-[#202020] bg-opacity-25 text-white hover:bg-opacity-25"} border-white border-opacity-25 border-2`}
              onClick={() => {
            ...
            >
              ...
              <div className="mt-1 opacity-50">
                <h3 className="mt-2 text-sm">For sending packages around the country</h3>
              </div>
          ...
          {showPriceDetails && (
            <div className="mt-4 p-4 rounded-lg flex flex-row gap-3 justify-center">
              <div id="map" ref={mapContainerRef} className="w-1/2 h-36 rounded-lg -translate-x-2"></div>
              <div className="flex flex-col bg-[#202020] opacity-75 rounded-xl p-4 w-80">
                {serviceType === "Pickup & Drop" ? (
                  <>
                    <p className="text-sm mb-2">Trip Fare ({distance?.toFixed(1)} kms)</p>
                  ...
                  <>
                    ...
                    <div className="flex flex-row justify-between text-xs gap-4">
                      ...
                      <p>₹{(amount / 1.18).toFixed(2)}</p> {/* Subtracting GST for display */}
                    </div>
                    <div className="flex flex-row justify-between text-xs">
                      <span>GST Charges:</span>
                      <p>₹{(amount - amount / 1.18).toFixed(2)}</p> {/* Displaying only GST */}
                    </div>
                    <div className="flex flex-row justify-between text-xs">
                      <span>Total:</span>
                      <p>₹{amount}</p>
          ...
          <Button
            ...
            onClick={priceCalculated ? () => router.push('/dashboard/booking/detail-address') : calculateFare}
          >
